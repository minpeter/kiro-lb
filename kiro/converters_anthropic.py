...
def split_inline_system_messages(messages: List[Any]) -> tuple[List[Any], List[str]]:
    """Separate inline ``role: "system"`` turns from the conversation.

    Anthropic's schema keeps the system prompt in a dedicated top-level field,
    but several clients inline it in ``messages``. Kiro rejects unknown roles
    with ``REQUEST_BODY_INVALID``, so hoist the text instead of forwarding it.

    Args:
        messages: Raw Anthropic messages.

    Returns:
        Tuple of (conversation messages, extracted system prompt fragments).
    """
    ...
    for message in messages:
        ...
        text = extract_system_prompt(content)
        if text:
...
def anthropic_to_kiro(
    ...
    """
    # Hoist any inline system turns before converting the conversation.
    conversation_messages, inline_system = split_inline_system_messages(request.messages)
    ...
    system_prompt = "\n\n".join(filter(None, [extract_system_prompt(request.system), *inline_system]))
    ...
    # Anthropic clients express reasoning through thinking/output_config. Kiro
    # only accepts the adaptive form, so translate instead of forwarding.
    apply_native_thinking(
        result.payload,
        model_id,

...
def build_kiro_payload(request_data: ChatCompletionRequest, conversation_id: str, profile_arn: str) -> dict:
    ...
    # prompt trick. Only an explicit client effort requests it, so ordinary
    # requests stay byte-for-byte equivalent to the plain payload path.
    apply_native_thinking(result.payload, model_id, request_data.reasoning_effort)

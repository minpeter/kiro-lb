...
class AnthropicMessage(BaseModel):
    ...
    Attributes:
        role: Message role. Anthropic documents only user/assistant, but some
            clients inline a system turn, so it is accepted here and hoisted
            into the top-level system prompt during conversion.
...
class AnthropicMessagesRequest(BaseModel):
    ...
    # Extended thinking (official Anthropic parameter). Newer models use
    # {"type": "adaptive"}; older clients send {"type": "enabled", ...}.
    thinking: Optional[Dict[str, Any]] = None

    # Effort-based reasoning control used by Claude Code on Opus 4.7+.

...
def get_model_id_for_kiro(
    ...
    ModelResolver. It normalizes the name, then resolves public aliases and
    hidden models to the identifier the upstream actually accepts.

    Aliases must be resolved here: an advertised alias such as "auto-kiro" is
    rejected upstream with INVALID_MODEL_ID unless it is mapped to "auto".

    Args:
        model_name: External model name from client
        hidden_models: Dict mapping display names to internal Kiro IDs
        aliases: Dict mapping public alias names to real model IDs
    ...
    if aliases:
        # Aliases point at real model ids, which may themselves need normalizing.
        target = aliases.get(normalized) or aliases.get(model_name)
        if target:

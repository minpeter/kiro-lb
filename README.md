...
**Private Kiro API load balancer for Amazon Q Developer / AWS CodeWhisperer**

> This AGPL-3.0 fork adds a private operations dashboard and intentionally does
> **not** fabricate reasoning from prompts or response tags. Native adaptive
> reasoning is forwarded only for validated Kiro upstream models; normal
> reasoning UI is never synthesized from response text.
...
| 📊 **Operations dashboard** | Password-protected account health, API-key management, and metadata-only request logs |
| 🧠 **Native adaptive reasoning** | Verified Kiro structured reasoning events; no prompt-tag fallback |
| 🔀 **Low-latency account failover** | 429s rotate the account pool immediately instead of sleeping through retries |
...
Copyright (C) 2025 Jwadow. Modifications are Copyright (C) 2026 minpeter and are
released under the same license. [NOTICE.md](NOTICE.md) lists the significant
changes, and [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) covers the
MIT-licensed dashboard frontend it vendors.

### Source for network users (AGPL section 13)

This program is normally run as a network service, so anyone interacting with a
deployed instance is entitled to the corresponding source of the running
version. It is published at <https://github.com/minpeter/kiro-lb>.

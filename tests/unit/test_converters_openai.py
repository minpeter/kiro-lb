...
class TestConvertOpenAIMessagesToUnified:
    ...
    def test_preserves_assistant_history_across_followup_turn(self):
        messages = [
            ChatMessage(role="user", content="Reply with TURN1_OK"),
            ChatMessage(
                role="assistant",
                content="TURN1_OK",
                reasoning_content="private upstream reasoning",
            ),
            ChatMessage(role="user", content="Reply with TURN2_OK"),
        ]

        _, unified = convert_openai_messages_to_unified(messages)

        assert [message.role for message in unified] == [
            "user",
            "assistant",
            "user",
        ]
        assert [message.content for message in unified] == [
            "Reply with TURN1_OK",
            "TURN1_OK",
            "Reply with TURN2_OK",
        ]
    ...
    def test_preserves_tool_result_continuation_sequence(self):
        messages = [
                ...
                tool_calls=[
                    {
                        ...
                        "function": {
                            "name": "echo_value",
                            "arguments": '{"value":"MULTITURN_ALPHA"}',
                        },
                    }
                ],
            ),
            ...
            ChatMessage(role="user", content="Reply with TOOL_CONTINUATION_OK"),
        ]
        ...
        assert unified[1].tool_calls[0]["id"] == "call_echo"
        assert unified[2].tool_results == [
            {
                "type": "tool_result",
